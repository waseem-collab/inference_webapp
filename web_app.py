...
try:
    PPE_DEFAULT_PROMPTS = json.loads(
        (Path(__file__).resolve().parent / "ppe_prompts.json").read_text(encoding="utf-8")
    )
...
<style>
/* ==================== LIQUID GLASS THEME ====================
   Perf: backdrop-filter is expensive, so ONLY the few big panels are blurred
   (never inputs/buttons/pills, and never anything over the live video). */
:root{
  --text:#f4f7ff; --muted:rgba(233,240,255,.74); --muted-2:rgba(233,240,255,.48);
  --panel:rgba(255,255,255,.085); --panel-2:rgba(255,255,255,.06); --panel-3:rgba(255,255,255,.12);
...
/* Kill ALL backdrop blur (incl. base rules that overlay the live video)… */
*{-webkit-backdrop-filter:none !important;backdrop-filter:none !important;}
/* …then re-enable it on ONLY the few big frosted panels. */
.card,.stage,.dd-menu{
  background:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.012) 46%),var(--panel);
  -webkit-backdrop-filter:blur(18px) saturate(160%) !important; backdrop-filter:blur(18px) saturate(160%) !important;
  border:1px solid var(--line-2); border-radius:20px;
  box-shadow:0 10px 30px rgba(0,0,0,.32), inset 0 1px 0 rgba(255,255,255,.22);
}
.stage{border-radius:22px;}
/* Toolbar overlays the live video — translucent, blur OFF (no per-frame cost). */
.toolbar,.fs-controls{background:rgba(10,13,22,.74) !important;border:1px solid var(--line-2) !important;
  -webkit-backdrop-filter:none !important;backdrop-filter:none !important;}
.video-wrap{background:var(--stage) !important;border:1px solid rgba(255,255,255,.10) !important;}

/* Inner controls: translucent only, no per-element blur. */
input,select,textarea,.dd-btn,.ai-sec-prompt{
  background:rgba(255,255,255,.07) !important;border:1px solid var(--line-2) !important;color:var(--text) !important;border-radius:12px !important;
}
input:focus,select:focus,textarea:focus,.dd-btn:focus{
  border-color:rgba(255,255,255,.45) !important;box-shadow:0 0 0 3px rgba(255,255,255,.14) !important;
}
select option{background:#141a30;color:var(--text);}
.t-btn,.ghost-btn,.tool-btn,.back,.card-min,.toolbar-toggle,.ai-sec-save,.fs-t,.fs-btn{
  background:rgba(255,255,255,.09) !important;border:1px solid var(--line-2) !important;color:var(--text) !important;border-radius:12px !important;
}
.t-btn:hover,.ghost-btn:hover,.tool-btn:hover,.back:hover,.fs-t:hover{background:rgba(255,255,255,.18) !important;border-color:rgba(255,255,255,.4) !important;}
.pill{background:rgba(255,255,255,.11);border:1px solid var(--line-2);}
.pill.playing{color:#06210a;background:rgba(166,226,46,.85);border-color:rgba(166,226,46,.9);}
.toggle-row,.src-sec,.cls-all-row,.ai-sec,.ai-result{background:rgba(255,255,255,.06) !important;border:1px solid var(--line-2) !important;border-radius:12px !important;}
.src-sec.open{border-color:var(--hivis) !important;background:rgba(255,207,90,.09) !important;}
.seek-track{background:rgba(255,255,255,.14);}
.seek-seg{background:rgba(255,255,255,.35);}
.dd-menu{background:rgba(18,22,42,.82) !important;}
</style>
"""
...
@app.get("/")
def index():
    return _render_inference_page("glass")


def _render_inference_page(theme="glass"):
    with state_lock:
...
<style>
:root{
  --bg:#0a0c11; --stage:#04060a;
  --panel:#12151d; --panel-2:#171b24; --panel-3:#1b202c;
  --line:#232a37; --line-2:#313a4b;
  --text:#eef2f8; --muted:#8b93a5; --muted-2:#616a7d;
  --hivis:#ffb200; --hivis-2:#ffcb45; --hivis-soft:rgba(255,178,0,.14);
  --go:#a6e22e; --go-deep:#7fb800;
  --bad:#f2555a; --ok:#35d07f;
...
body{
  ...
  background:
    radial-gradient(1200px 700px at 82% -12%, rgba(255,178,0,.055), transparent 60%),
    radial-gradient(900px 620px at -12% 112%, rgba(120,160,255,.05), transparent 55%),
    var(--bg);
  overflow:hidden;
}
...
.brand-dot.on{background:var(--go);box-shadow:0 0 12px 1px rgba(166,226,46,.6);animation:pulse 1.8s ease-in-out infinite;}
@keyframes pulse{50%{box-shadow:0 0 5px 0 rgba(166,226,46,.3);}}
...
.pill.playing{color:#06210a;background:var(--go);border-color:var(--go);font-weight:700;}
.pill.err{background:rgba(242,85,90,.16);border-color:var(--bad);color:#ffb4b6;max-width:38vw;overflow:hidden;text-overflow:ellipsis;}
...
/* Home button: a distinct hi-vis pill so "return to the app picker" always reads
   as the primary way out, not just another ghost button. */
.home-btn{display:inline-flex;align-items:center;gap:8px;font-family:var(--sans);font-size:12.5px;font-weight:700;
  letter-spacing:.2px;text-decoration:none;color:var(--hivis);cursor:pointer;flex:0 0 auto;
...
.home-btn:hover{color:#0a0d1a;background:var(--hivis);border-color:var(--hivis);
...
.video-wrap{position:relative;flex:1 1 auto;min-height:0;display:flex;border-radius:12px;overflow:hidden;background:var(--stage);border:1px solid #10151f;}
.video{width:100%;height:100%;object-fit:contain;display:block;cursor:default;}
.video.pick-mode{cursor:crosshair;}

.toolbar{position:absolute;top:12px;right:12px;z-index:10;display:flex;flex-direction:column;gap:6px;width:182px;background:rgba(10,13,19,.72);backdrop-filter:blur(9px);-webkit-backdrop-filter:blur(9px);border:1px solid var(--line-2);border-radius:11px;padding:9px;}
...
.fs-btn{position:absolute;bottom:12px;right:12px;z-index:12;width:34px;height:34px;padding:0;border-radius:9px;cursor:pointer;
  background:rgba(10,13,19,.72);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);border:1px solid var(--line-2);
...
.fs-t{width:auto;height:44px;padding:0 18px;border-radius:10px;cursor:pointer;font-size:14.5px;color:var(--text);
  background:rgba(23,27,36,.85);border:1px solid var(--line-2);display:inline-flex;align-items:center;justify-content:center;
  transition:border-color .14s,background .14s;}
.fs-t:hover{border-color:var(--hivis);background:rgba(27,32,44,.95);}
.fs-play{width:60px;height:60px;flex:0 0 auto;padding:0;border:none;border-radius:50%;color:#06210a;cursor:pointer;
  background:radial-gradient(120% 120% at 30% 25%, var(--go), var(--go-deep));box-shadow:0 6px 22px rgba(126,184,0,.45);
...
.fs-play.playing{color:#241500;background:radial-gradient(120% 120% at 30% 25%, var(--hivis), #cc8f00);box-shadow:0 6px 22px rgba(255,178,0,.45);}
.video-wrap:fullscreen{background:#000;border-radius:0;border:none;}
...
.video-wrap:fullscreen .fs-controls{display:flex;align-items:center;gap:14px;position:absolute;left:50%;bottom:34px;
  ...
  background:rgba(10,13,19,.62);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.08);
  box-shadow:0 12px 40px rgba(0,0,0,.5);transition:opacity .25s;}
...
.tool-btn.dl{border-color:#2c3a4e;color:#bcd3ff;}
.tool-btn.dl:hover{border-color:#4d7fff;background:var(--panel-3);}
.tool-btn.cancel{border-color:var(--bad);color:#ffb4b6;}
.kbd{font-family:var(--mono);font-size:10px;color:var(--muted);background:#0c0f16;border:1px solid var(--line-2);border-bottom-width:2px;border-radius:4px;padding:1px 5px;min-width:20px;text-align:center;}
...
.draw-box{position:absolute;border:2px solid var(--hivis);background:rgba(255,178,0,.14);pointer-events:none;display:none;}
.video-note{position:absolute;left:12px;bottom:12px;z-index:11;max-width:60%;font-size:12.5px;color:#eef2f8;background:rgba(10,13,19,.9);border:1px solid var(--line-2);border-left:3px solid var(--hivis);border-radius:8px;padding:8px 11px;display:none;}
...
.play-btn{width:56px;min-width:56px;height:56px;flex:0 0 auto;padding:0;border:none;border-radius:50%;color:#06210a;cursor:pointer;background:radial-gradient(120% 120% at 30% 25%, var(--go), var(--go-deep));box-shadow:0 6px 20px rgba(126,184,0,.4),inset 0 1px 0 rgba(255,255,255,.25);display:inline-flex;align-items:center;justify-content:center;transition:transform .1s,box-shadow .18s,background .2s,filter .12s;}
.play-btn:hover{filter:brightness(1.06);transform:translateY(-1px);}
.play-btn:active{transform:translateY(0);}
.play-btn.playing{color:#241500;background:radial-gradient(120% 120% at 30% 25%, var(--hivis), #cc8f00);box-shadow:0 6px 20px rgba(255,178,0,.42),inset 0 1px 0 rgba(255,255,255,.25);}
...
.dl-ring-bg{fill:none;stroke:rgba(255,255,255,.12);stroke-width:4;}
...
.slider-wrap input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;width:16px;height:16px;margin-top:3px;border-radius:50%;background:#fff;border:3px solid var(--hivis);cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.5);}
.slider-wrap input[type=range]::-moz-range-thumb{width:14px;height:14px;border-radius:50%;background:#fff;border:3px solid var(--hivis);cursor:pointer;}
.seek-track{position:absolute;left:0;right:0;top:50%;transform:translateY(-50%);height:6px;border-radius:5px;background:#1a2130;overflow:hidden;z-index:0;}
.seek-seg{position:absolute;top:0;height:100%;background:#39435a;}
...
.card-title{font-size:11px;letter-spacing:1.4px;text-transform:uppercase;color:#c7cedb;font-weight:700;}
...
.ai-send{width:100%;min-height:40px;border:none;border-radius:9px;cursor:pointer;font-weight:700;font-size:14px;
  color:#06210a;background:linear-gradient(180deg,var(--go),var(--go-deep));box-shadow:0 4px 14px rgba(126,184,0,.28);
  transition:filter .12s,background .2s;}
.ai-send:hover{filter:brightness(1.06);}
.ai-send.picking{background:linear-gradient(180deg,var(--hivis),#cc8f00);color:#241500;}
.ai-note{font-size:12px;color:var(--muted);line-height:1.5;}
.ai-note b{color:#c7cedb;}
...
.ai-result.err{border-color:var(--bad);color:#ffb4b6;}
...
.dd-menu{position:fixed;z-index:60;max-height:300px;background:var(--panel-3);border:1px solid var(--line-2);
  border-radius:10px;padding:6px;box-shadow:0 18px 44px rgba(0,0,0,.6);display:none;flex-direction:column;}
...
.url-play{flex:0 0 auto;width:auto;height:36px;padding:0 16px;border:none;border-radius:8px;cursor:pointer;
  color:#06210a;font-weight:700;font-size:13px;white-space:nowrap;
  background:linear-gradient(180deg,var(--go),var(--go-deep,#7fb800));box-shadow:0 3px 12px rgba(126,184,0,.3);}
...
.cls-panel{position:absolute;right:0;top:calc(100% + 7px);z-index:40;width:232px;max-height:300px;overflow:hidden;display:flex;flex-direction:column;background:var(--panel-3);border:1px solid var(--line-2);border-radius:11px;padding:8px;box-shadow:0 18px 44px rgba(0,0,0,.6);}
...
/* Toast */
.toast{position:fixed;left:50%;bottom:24px;transform:translateX(-50%) translateY(10px);z-index:2000;font-size:13px;font-weight:600;color:#06210a;background:var(--go);border:1px solid var(--go-deep);border-radius:10px;padding:10px 16px;opacity:0;pointer-events:none;box-shadow:0 10px 30px rgba(0,0,0,.4);transition:opacity .16s,transform .16s;}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0);}
.toast.error{color:#fff;background:var(--bad);border-color:#c0343a;}
...
<div class="app">
  <header class="topbar">
    ...
    <a class="home-btn" href="/home" title="Back to the app picker"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V20a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1V9.5"/><path d="M9.5 21v-6h5v6"/></svg>Home</a>
...
"""
    ...
    if theme == "glass":
        page = page.replace("</head>", GLASS_CSS + "</head>", 1)
